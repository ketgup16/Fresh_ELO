        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--ld-semantic-font-family-sans)', fontSize: '14px' }}>
              ...
              <tbody>
                {[
                  ['eyebrow',         'string',                                     'undefined', 'Optional text displayed above the title.'],
                  ['title',           'string',                                     '—',        'Required. Primary label using body-large bold token.'],
                  ['text',            'string',                                     'undefined', 'Optional secondary text below the title.'],
                  ['trailing',        "'empty' | 'icon' | 'link' | 'select'",       "'empty'",  'Trailing slot variant.'],
                  ['trailingIcon',    'ReactNode',                                  'ChevronRight', 'Custom icon for trailing="icon".'],
                  ['trailingLink',    '{ text: string; onClick?: () => void }',     'undefined', 'Link config for trailing="link".'],
                  ['attributes',      'Array<{ label: string; icon?: ReactNode }>', 'undefined', 'Optional AXAttribute Small items (up to 3).'],
                  ['tag',             'ListItemTagPreset | ListItemTagCustom',       'undefined', 'Tag slot. Preset: "unassigned" | "assigned" | "complete". Custom: { variant, color, label }.'],
                  ['divider',         'boolean',                                    'undefined', 'Renders a Divider at the bottom with 16px top margin.'],
                  ['alert',           'ReactNode',                                  'undefined', 'Alert node rendered below attributes.'],
                  ['footerAction',    'ReactNode',                                  'undefined', 'Optional full-width action below the content.'],
                  ['className',       'string',                                     'undefined', 'Additional CSS class names on the <li> element.'],
                ].map(([prop, type, def, desc]) => (
                  <tr key={prop} style={{ borderBottom: '1px solid var(--ld-semantic-color-border-subtlest)' }}>
                    <td style={{ padding: '8px 12px', fontFamily: 'monospace', fontSize: '13px', color: 'var(--ld-semantic-color-text-brand)', whiteSpace: 'nowrap' }}>{prop}</td>
                    <td style={{ padding: '8px 12px', fontFamily: 'monospace', fontSize: '12px', color: 'var(--ld-semantic-color-text-subtle)' }}>{type}</td>
