...
import { AXCountry, AX_DEFAULT_COUNTRIES, AXCountrySelectBottomSheet } from './AXCountrySelectBottomSheet';
import styles from './AXCountryCodePhoneInput.module.css';

export interface AXCountryCodePhoneInputProps {
  /** Field label */
  label?: string;
  /** Phone number value */
  value?: string;
  ...
  selectedCountry?: AXCountry;
  /** Called when country changes */
  onCountryChange?: (country: AXCountry) => void;
...
export function AXCountryCodePhoneInput({
  label = 'Phone number*',
  value = '',
  onChange,
  selectedCountry: controlledCountry,
  onCountryChange,
  countries = AX_DEFAULT_COUNTRIES,
  disabled = false,
  readOnly = false,
  error = false,
  ...
  id,
}: AXCountryCodePhoneInputProps) {
  const [sheetOpen, setSheetOpen] = React.useState(false);
  const [internalCountry, setInternalCountry] = React.useState<AXCountry>(
    countries.find(c => c.code === 'US') ?? countries[0]
  );

  const selectedCountry = controlledCountry ?? internalCountry;

  const handleCountryConfirm = (country: AXCountry | undefined) => {
    if (!country) return;
