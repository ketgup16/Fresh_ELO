...
import displayStyles from './AXRatingDisplay.module.css';

export type AXRatingSize = 'small' | 'medium';
export type AXRatingColor = 'default' | 'inverse';

export interface AXRatingProps {
   ...
   * (equivalent to the former AXRatingDisplay component).
   */
...
/**
 * AX Rating — Interactive star rating component
 *
 ...
 * @example
 * ```tsx
 * <AXRating size="medium" onChange={(v) => console.log(v)} />
 * <AXRating size="small" value={3} />
 * ```
 */
export const AXRating = React.forwardRef<HTMLDivElement, AXRatingProps>(
  (props, ref) => {
    const {
    ...
    // ── Read-only / display mode (equivalent to AXRatingDisplay) ───────────
    if (isDisplayMode) {
