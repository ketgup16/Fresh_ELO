...
export interface AXSearchFieldProps {
  ...
  /**
   * Height variant.
   * - xsmall: 32px — small icons & icon buttons
   * - small:  40px — small icons & icon buttons
   * - medium: 48px — medium icons & icon buttons
...
const PILL_SIZE_CLASS: Record<AXSearchFieldSize, string> = {
  xsmall: styles.pillXSmall,
...
export function AXSearchField({
  ...
  // xsmall + small use small icon buttons & 20px search icon; medium + large use medium & 24px
  const isSmallVariant = size === 'xsmall' || size === 'small';
  const iconBtnSize = isSmallVariant ? 'small' : 'medium';
