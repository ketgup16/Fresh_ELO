...
import { AXFlag, AX_FLAG_VARIANTS, type AXFlagVariant } from '@/components/walmart/AXFlag';
import {
  Gift,
  Star,
  Hourglass,
...
const VARIANT_ICONS: Record<AXFlagVariant, React.ReactNode> = {
...
const BG_TOKENS: Record<AXFlagVariant, string> = {
...
const TEXT_TOKENS: Record<AXFlagVariant, string> = {
...
export default function AXFlagPage() {
  return (
    <ComponentPageLayout
    ...
    >
      <div>
        ...
        <section style={{ marginBottom: '56px' }}>
          ...
          <div style={{
            ...
            {AX_FLAG_VARIANTS.map(({ variant, label }) => (
              ...
              >
                <AXFlag variant={variant} label={label} leadingIcon={VARIANT_ICONS[variant]} />
                <span style={{
                  fontSize: '11px',
                    ...
                    {rows.map((rule, i) => (
                      ...
                      >
                        ...
                        <div style={{ paddingTop: '2px' }}>
                          <AXFlag variant={rule.variant} label={rule.label} leadingIcon={rule.icon} />
                        </div>
        ...
        <section style={{ marginBottom: '56px' }}>
          ...
          <p style={subheadStyle}>
            Flags appear in the top-left corner of product tiles (AXItemTile) as an absolute overlay over
          ...
          <div style={{
            ...
            {[
              { label: 'Best seller', variant: 'savings-bold' as AXFlagVariant, icon: <Dollar {...ICON_SIZE} /> },
              { label: 'Rollback',    variant: 'holiday-restricted' as AXFlagVariant, icon: undefined },
              { label: 'Deal',        variant: 'savings-subtle' as AXFlagVariant, icon: <Tag {...ICON_SIZE} /> },
              { label: 'Popular',     variant: 'confidence-subtle' as AXFlagVariant, icon: <ShieldCheck {...ICON_SIZE} /> },
              { label: 'Clearance',   variant: 'urgent' as AXFlagVariant, icon: <Flash {...ICON_SIZE} /> },
              { label: 'Only 2 left!', variant: 'scarcity' as AXFlagVariant, icon: <Hourglass {...ICON_SIZE} /> },
            ...
            {AX_FLAG_VARIANTS.map((meta, i) => (
              <div
                ...
                style={{
                  ...
                  padding: '12px 16px',
                  alignItems: 'center',
                  borderBottom: i < AX_FLAG_VARIANTS.length - 1
                    ? '1px solid var(--ld-semantic-color-separator, #e3e4e5)'
                    : 'none',
                }}
              >
                <div><AXFlag variant={meta.variant} label="Flag name" /></div>
        ...
        <section>
          ...
          <div style={{
            ...
            {AX_FLAG_VARIANTS.map(m => (
              <AXFlag key={m.variant} variant={m.variant} label={m.label} leadingIcon={VARIANT_ICONS[m.variant]} />
            ))}
          </div>
        </section>
    ...
    <div style={{
      ...
      <div style={{ position: 'relative', height: '120px', background: 'var(--ld-semantic-color-fill-surface-secondary, #f5f5f6)' }}>
        <div style={{
          ...
          <AXFlag variant={variant} label={label} leadingIcon={icon} />
        </div>
